...
const listFoods = (recipe) => {
  ...
  const locateSpace = ' ';
  // Solution code here...
  $(recipe.ingredients).each(index => {
    let ingred = recipe.ingredients[index];
    let indexFirstSpace = ingred.indexOf(locateSpace);
    let locateSecondSpace = ingred.indexOf(locateSpace, (indexFirstSpace + 1));
    result.push(ingred.slice(locateSecondSpace + 1));
  });
  return result;
};
