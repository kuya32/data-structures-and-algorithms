...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let vowels = arr.filter(vow => {
    let regEx = /\w+[aeiou]\w+|[aeiou]/g
    let findThatIsh = vow.match(regEx);
    return findThatIsh;
  });
  return vowels;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let arrays = arr.filter(datTing => {
    let notEqual = !forbiddenValues.includes(datTing);
    return notEqual;
  });
  return arrays;
};
