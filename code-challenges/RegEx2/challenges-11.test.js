...
const toLastNames = people => {
  // Solution code here...
  let peeps = people.map((val) => `${val.firstName} ${val.lastName}`);
  return peeps;
};
...
const validateEmail = (email) => {
  ...
  let emailChecker = regEx.test(email);
  return emailChecker;
};
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  let phoneChecker = regEx.test(phoneNumber);
  return phoneChecker;
};
