...
const toTitleCase = (arr) => {
  ...
  let letter = arr.map(str => {
    let grabsFirstLowercaseLetter = str.substring(0, 1);
    let capitalizeLowercase = (grabsFirstLowercaseLetter).toUpperCase()
    let switchLowWithCap = str.replace(str.substring(0, 1), capitalizeLowercase);
    return switchLowWithCap;
  });
  return letter;
};
...
//   // Solution code here...
//   let bigBoys = arr.filter(peeps => {
//     if (peeps.mass > 78){
//       return peeps;
//     }
//   });

//   let bigBoyNames = bigBoys.map(names => {
//     return names.name
//   });

//   return bigBoyNames.join(' - ');
// };

let biggerThanLuke = (arr) => arr.filter(peeps => parseInt(peeps.mass) > 78).map(val => val.name).join().replace(',',' - ');
